...
from config import get_current_round, get_round_avgs
...
if CURRENT_ROUND not in ROUND_AVGS:
    raise SystemExit(
        f"ERROR: No round average configured for round {CURRENT_ROUND}. "
        "Pass it through run_all.py, set ROUND_AVG_CURRENT, or update round_avgs.json."
    )
# ════════════════════════════════════════════════════════════════
...
SPECIAL_TAGS = {
    "Don Cominos":      {"label": "League Commissioner", "icon": "⚙️"},
...
output = {
    "meta": {
        ...
        "tradesCapacity":        TRADES_CAPACITY,
        "tierBoundaries":        {"t1": 100, "t2": 200},
        "tradesRules":           {"base": TRADES_BASE, "gifted": TRADES_GIFTED, "capacity": TRADES_CAPACITY},
