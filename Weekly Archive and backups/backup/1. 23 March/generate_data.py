...
# ════════════════════════════════════════════════════════════════
CURRENT_ROUND = 2          # ← bump to 3 when R3 data is ready
ROUND_AVGS    = {1: 926, 2: 1034}   # ← add 3: XXXX when R3 is complete
# ════════════════════════════════════════════════════════════════
...
SPECIAL_TAGS = {
    ...
    "Andrew Phillipps": {"label": "2025 Winner",   "icon": "✦"},
    "Jake Venn":        {"label": "2025 Runner-Up","icon": "✦"},
    "Jamie Brown":      {"label": "Admin",          "icon": "🎙️", "isAdmin": True},
    "Mitch Deguara":    {"icon": "🎙️"},
    "Nikki Mcintosh":   {"icon": "🎙️"},
    "Matt Thompson":    {"icon": "🎙️"},
...
prev_full = scrapes_full[CURRENT_ROUND - 1]
...
all_pids = set()
for r in range(1, CURRENT_ROUND + 1):
    all_pids |= set(scrapes_full[r]["player_id"].unique())

players_global = {}
for pid in all_pids:
    pid_int  = int(pid)
    pid_str  = str(pid_int)
    info     = pname_map.get(pid_int, {"name": str(pid_int), "cost": 0})
    ti       = traded_in_count.get(pid, 0)
    to_      = traded_out_count.get(pid, 0)
    prev_own = int(ownership_prev.get(pid, 0))
    cur_own  = int(ownership_cur.get(pid, 0))
    ...
    players_global[pid_str] = {
        ...
        "cost":         info["cost"],
        "owned":        cur_own,
        "ownedPct":     float(ownership_pct.get(pid, 0.0)),
        "tradedIn":     ti,
        "tradedOut":    to_,
...
# ── INSIGHTS SIGNAL LAYER ────────────────────────────────────
# Appended after players_global is fully populated.
# Uses only data already available per player — no new sources.
# Three signals: mispricing_index, role_shift, momentum.

DYNAMIC_MAGIC_NUMBER = K_MAGIC  # 13000 — price-per-point multiplier

for pid_str, p in players_global.items():
    gp            = p.get("gamesPlayed", 0) or 0
    ordered       = p.get("orderedScores", []) or []
    current_price = p.get("currentPrice") or 0
    break_even    = p.get("breakEven")

    # ── Defaults (insufficient data) ──────────────────────────
    signals = {
        "mispricing_index": 0.0,
        "mispricing_label": "FAIR",
        "role_shift_label": "STABLE",
        "role_shift_delta": 0.0,
        "momentum_label":   "STABLE",
        "momentum_score":   0.0,
    }

    if gp < 2 or not ordered or current_price == 0:
        p["signals"] = signals
        continue

    # ── SIGNAL 1: PRICE MISPRICING INDEX ──────────────────────
    season_avg  = sum(ordered) / len(ordered)
    last3       = ordered[-3:] if len(ordered) >= 3 else ordered
    last3_avg   = sum(last3) / len(last3)
    exp_score   = (0.7 * last3_avg) + (0.3 * season_avg)
    implied_val = exp_score * DYNAMIC_MAGIC_NUMBER
    mispricing  = implied_val - current_price
    mis_index   = round(mispricing / current_price, 3) if current_price else 0.0

    if mis_index > 0.10:
        mis_label = "UNDERPRICED"
    elif mis_index < -0.10:
        mis_label = "OVERPRICED"
    else:
        mis_label = "FAIR"

    # ── SIGNAL 2: ROLE SHIFT DETECTOR ─────────────────────────
    recent_2    = ordered[-2:]
    prior_35    = ordered[-5:-2] if len(ordered) >= 3 else []
    recent_avg  = sum(recent_2) / len(recent_2)
    baseline_b  = sum(prior_35) / len(prior_35) if prior_35 else None

    if baseline_b and baseline_b != 0:
        role_delta = round(recent_avg - baseline_b, 1)
        delta_ratio = role_delta / baseline_b
        if delta_ratio > 0.20:
            role_label = "ROLE_UP"
        elif delta_ratio < -0.20:
            role_label = "ROLE_DOWN"
        else:
            role_label = "STABLE"
    else:
        role_delta = 0.0
    ...
    # ── SIGNAL 3: MOMENTUM VS REGRESSION ──────────────────────
    short_avg    = sum(ordered[-2:]) / len(ordered[-2:])
    window_5     = ordered[-5:] if len(ordered) >= 2 else ordered
    base5_avg    = sum(window_5) / len(window_5)
    trend_delta  = short_avg - base5_avg
    be_gap       = (short_avg - break_even) if break_even is not None else 0.0
    mom_score    = round((0.6 * trend_delta) + (0.4 * be_gap), 1)

    if mom_score > 5:
        mom_label = "ACCELERATING"
    elif mom_score < -5:
        mom_label = "REGRESSING"
    else:
        mom_label = "STABLE"

    p["signals"] = {
        "mispricing_index": mis_index,
        "mispricing_label": mis_label,
        "role_shift_label": role_label,
        "role_shift_delta": role_delta,
