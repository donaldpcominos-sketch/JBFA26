...
squad_id_map   = {}   # pid → NRL squad_id (int)
...
if os.path.exists("players.json"):
    ...
    for p in raw_players:
        ...
        if p.get("squad_id"):
            squad_id_map[str(p["id"])] = p["squad_id"]
...
for pid_str in all_pids_str:
    ...
    players_global[pid_str] = {
        ...
        # NRL squad ID for bye planner
        "squadId": squad_id_map.get(pid_str, None),
    }
...
# ════════════════════════════════════════════════════════════════
#  BYE PLANNER — schedule + Origin config
# ════════════════════════════════════════════════════════════════
bye_schedule: dict[str, list[int]] = {}   # round(str) → [squad_ids on bye]
squad_names:  dict[str, str]       = {}   # squad_id(str) → short name

if os.path.exists("nrl_schedule.json"):
    with open("nrl_schedule.json") as f:
        nrl_sched = json.load(f)
    for rd in nrl_sched:
        byes = rd.get("bye_squads", [])
        if byes:
            bye_schedule[str(rd["id"])] = byes
        for m in rd.get("matches", []):
            squad_names[str(m["home_squad_id"])] = m["home_squad_name"]
            squad_names[str(m["away_squad_id"])] = m["away_squad_name"]
else:
    print("WARNING: nrl_schedule.json not found — bye planner data will be empty")

# Origin rounds + default player list
# Update these each year. Origin rounds = the NRL rounds during Origin.
# originDefaults = player IDs likely selected for Origin duty.
ORIGIN_ROUNDS = [15, 16, 18]  # 2026 Origin series rounds (adjust yearly)
ORIGIN_DEFAULTS: dict[str, bool] = {
    # Populated per season — these are player IDs (str) of likely Origin picks.
    # Leave empty if not yet known; users can add manually in the planner.
}
...
output = {
    ...
    # Bye planner data
    "byeSchedule":    bye_schedule,      # round → [squad_ids on bye]
    "squadNames":     squad_names,       # squad_id → short team name
    "originRounds":   ORIGIN_ROUNDS,     # round numbers with Origin
    "originDefaults": ORIGIN_DEFAULTS,   # playerId → true
}
